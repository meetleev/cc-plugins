...
class ResMgr {
     ...
     */
    loadTiledMap(paths: string): Promise<TiledMapAsset> {
        return this.load(TiledMapAsset, `tiledMaps/${paths}`);
    }
     ...
     */
    loadAudio(paths: string): Promise<AudioClip> {
        return this.load(AudioClip, `audios/${paths}`);
    }
     ...
     * @api public
     * @example
     * // load the prefab (${project}/assets/resources/prefabs/audio.mp3) from resources
     * ccx.resMgr.loadPrefab('audio').then( (prefab) => console.log(prefab) ).catch( (err) => console.log(err) );
     */
    loadPrefab(paths: string): Promise<Prefab> {
        return this.load(Prefab, `prefabs/${paths}`);
    }
     ...
     */
    loadTexture(paths: string): Promise<Texture2D> {
        return this.load(Texture2D, `textures/${paths}/texture`);
    }
     ...
     */
    loadSpriteFrame(paths: string): Promise<SpriteFrame> {
        return new Promise((resolve, reject) => {
            this.load(SpriteFrame, `textures/${paths}/spriteFrame`, true).then((sp: SpriteFrame) => resolve(sp)).catch(
                () => {
     ...
     */
    loadMaterial(paths: string): Promise<Material> {
        return this.load(Material, `materials/${paths}`);
    }
     ...
     * @api public
     * @example
     * // load the spine (${project}/assets/resources/texture2D/bg.json) from resources
     * ccx.resMgr.loadSkeletonData('bg').then( (spine) => console.log(spine) ).catch( (err) => console.log(err) );
     */
    loadSkeletonData(paths: string): Promise<sp.SkeletonData> {
        return new Promise((resolve, reject) => {
            resources.load(`spines/${paths}`, sp.SkeletonData, (err, skeleton: sp.SkeletonData) => {
